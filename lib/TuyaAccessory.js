...
class TuyaAccessory extends EventEmitter {
  constructor(props) {
    ...
    if (!(props.id && props.key && props.ip) && !props.fake)
      return console.log("[Tuya] Insufficient details to initialize:", props);
    ...
    this._msgQueue = async.queue(
      this[
        this.context.version < 3.2 ? "_msgHandler_3_1" : "_msgHandler_3_3"
      ].bind(this),
      1
    );
  ...
  _send_3_3(o) {
    ...
    payload.writeInt32BE(
      getCRC32(payload.slice(0, payload.length - 8)),
      payload.length - 8
    );
