const BaseAccessory = require('homebridge-tuya/lib/BaseAccessory');
...
class SimpleFloorHeaterAccessory extends BaseAccessory {
    ...
    _getCurrentHeaterCoolerState(dps) {
        const {Characteristic} = this.hap;
        let dpHeatingState = this._getCustomDP(this.device.context.dpHeatingState) || 118;

        if (dps.hasOwnProperty(dpHeatingState) && dps[dpHeatingState] == 'warming') {
            return Characteristic.CurrentHeaterCoolerState.INACTIVE;
        }
        else if (dps.hasOwnProperty(dpHeatingState) && dps[dpHeatingState] == 'heating') {
